...
GENRE_WHITELIST = {
    "rock", "pop", "metal", "hip hop", "rap", "jazz", "blues", "electronic",
    "dance", "house", "techno", "trance", "indie", "indie rock", "alternative",
    "punk", "folk", "country", "r&b", "soul", "reggae", "classical", "soundtrack",
    "disco"
}
...
def normalize_tag(tag_name: str) -> str:
    ...
    if name in GENRE_WHITELIST:
        return name
    for g in GENRE_WHITELIST:
        if g in name:
            return g
    return ""

def extract_genres(tags, max_tags=5):
    if not tags:
        return [], None
    candidates = []
    for t in tags:
        ...
        if not raw_name:
            continue
        genre = normalize_tag(raw_name)
        if not genre:
            continue
        count = int(t.get("count", t.get("taggings", 0)) or 0)
        candidates.append((genre, count))
    if not candidates:
        return [], None
    agg = {}
    for g, c in candidates:
        agg[g] = agg.get(g, 0) + c
    sorted_genres = sorted(agg.items(), key=lambda x: x[1], reverse=True)
    genres = [g for g, _ in sorted_genres[:max_tags]]
    primary = genres[0] if genres else None
    return genres, primary

def get_track_tags(track_name, artist_name):
    params = {"method": "track.gettoptags", "track": track_name, "artist": artist_name, "autocorrect": 1}
    data = _safe_request(params)
    if not data:
        return []
    tags = data.get("toptags", {}).get("tag", [])
    return [tags] if isinstance(tags, dict) else tags

def get_artist_tags(artist_name):
    params = {"method": "artist.gettoptags", "artist": artist_name, "autocorrect": 1}
    data = _safe_request(params)
    if not data:
        return []
    tags = data.get("toptags", {}).get("tag", [])
...
def augment_songs_with_genre(songs_csv, artists_csv, out_csv, sleep_between_calls=0.2):
    songs_df = pd.read_csv(songs_csv, dtype=str)
    artists_df = pd.read_csv(artists_csv, dtype=str)[["artist_id", "artist_name"]]
    merged = songs_df.merge(artists_df, on="artist_id", how="left")

    genres_primary, genres_all = [], []
    artist_cache = {}

    for _, row in merged.iterrows():
        track_name = str(row.get("song_name", "") or "").strip()
        artist_name = str(row.get("artist_name", "") or "").strip()
        primary, all_genres = None, []

        if track_name and artist_name:
            time.sleep(sleep_between_calls)
            tags = get_track_tags(track_name, artist_name)
            all_genres, primary = extract_genres(tags)

        if not primary:
            key = artist_name.lower()
            if key in artist_cache:
                all_genres, primary = artist_cache[key]
            elif artist_name:
                time.sleep(sleep_between_calls)
                tags = get_artist_tags(artist_name)
                all_genres, primary = extract_genres(tags)
                artist_cache[key] = (all_genres, primary)

        if not primary:
            primary = "unknown"

        genres_primary.append(primary)
        genres_all.append(";".join(all_genres) if all_genres else "")
        print(f"Assigned genres for '{track_name}' by '{artist_name}': primary='{primary}', all={all_genres}")

    merged["genre_primary"] = genres_primary
    merged["genre_all"] = genres_all
    merged.to_csv(out_csv, index=False, encoding="utf-8")
    print(f"Saved songs with genre to {out_csv}")

if __name__ == "__main__":
    augment_songs_with_genre(
        songs_csv="dataset/song.csv",
        artists_csv="dataset/artist.csv",
        out_csv="dataset/songV2.csv",
        sleep_between_calls=0.002,
    )